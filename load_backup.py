...
def get_sys_name(ip=""):
    try:
        xml = requests.get(
            f"http://{ip}/getxml?location=/Configuration/SystemUnit/Name",
            headers=headers,
            verify=False,
            timeout=(10, 30),
        )
...
def load_backup(codec, sup_file):
    ...
    try:
        ...
        backup_fetch_status = http_request(
            codec.ip, fetch_backup_XML(sup_file["filename"], sup_file["checksum"])
        )
        ...
        if backup_fetch_status.find('<ServiceFetchResult status="OK">') != -1:
            ...
            codec.result = f"{sys_name} - Changes made successfully"
            return codec
        else:
            message(f"Could load backup for {sys_name}. Please investigate", sys_name)
            codec.result = (
                f"Codec error loading backup on {sys_name}. Please investigate."
            )
            return codec
    except Exception as err:
        codec.result = f"Loading backup process failed. Reason: {err}"
        return codec
